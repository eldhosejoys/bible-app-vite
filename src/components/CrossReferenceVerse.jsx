...
function CrossReferenceVerse({ to, fullBibleData, titles }) {
    ...
    const linkTo = (endBook && startBook === endBook && startChapter === endChapter && startVerse !== endVerse)
        ? `/${startBook}/${startChapter}/${startVerse}-${endVerse}`
        : `/${startBook}/${startChapter}/${startVerse}`;


    return (
        <div className="p-2 mt-2 border rounded" style={{ color: '#000', backgroundColor: '#faebd7' }}>
            <p className="fw-bold mb-1"><Link to={linkTo} className="text-decoration-none text-danger" title={referenceTitle}>{referenceTitle}</Link></p>
