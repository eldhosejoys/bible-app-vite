...
function Search() {
    ...
    }
    {
      (() => {
        ...
        if (r.length <= 0) {
          ...
          td.push(
            <div className="text-center fw-lighter mb-3 text-secondary">Search word found in {r.length} verses</div>
          );
        }
        else {
          var range;
          ...
          td.push(
            <div className="text-center fw-lighter mb-3 text-secondary">Showing the {range} verses of {r.length} with the search word</div>
          );
        }
  ...
  useEffect(() => {
    console.log('state changed', cards);
  }, [cards])
