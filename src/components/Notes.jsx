import { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import Modal from 'react-bootstrap/Modal';
import Button from 'react-bootstrap/Button';
import { getNotes, removeNote, addNote, getNoteDetails, onVerseStorageChange } from '../config/VerseStorage';
import { getTranslation } from '../config/SiteTranslations';
import { formatDateTime, getCacheData, addDataIntoCache, getLanguage, formatVerseRange } from '../config/Utils';
import { siteConfig, getBible } from '../config/siteConfig';
import axios from 'axios';

function Notes({ inModal = false, onNavigate }) {
    const [notes, setNotes] = useState([]);
    const [sortOrder, setSortOrder] = useState('newest');
    const [editingNote, setEditingNote] = useState(null);
    const [editContent, setEditContent] = useState('');
    const [bibleData, setBibleData] = useState(null);
    const [titlesData, setTitlesData] = useState(null);
    const [isLoading, setIsLoading] = useState(true);
    const [expandedItems, setExpandedItems] = useState({});

    useEffect(() => {
        if (!inModal) {
            document.title = `Notes | ${getTranslation().siteTitle}`;
        }

        const fetchData = async () => {
            try {
                const [titles, bible] = await Promise.all([
                    getCacheData('cache', siteConfig().titleurl).then(cached => cached || axios.get(siteConfig().titleurl).then(res => res.data)),
                    getCacheData('cache', getBible()).then(cached => cached || axios.get(getBible()).then(res => res.data))
                ]);

                if (titles) {
                    addDataIntoCache('cache', siteConfig().titleurl, titles);
                    setTitlesData(titles);
                }
                if (bible) {
                    addDataIntoCache('cache', getBible(), bible);
                    setBibleData(bible);
                }
            } catch (error) {
                console.error("Error loading data for notes:", error);
            } finally {
                setIsLoading(false);
            }
        };

        fetchData();
        fetchData();
        loadNotes();

        const unsubscribe = onVerseStorageChange(() => {
            loadNotes();
        });

        return () => unsubscribe();
    }, [inModal]);

    const loadNotes = async () => {
        const data = await getNotes();
        if (sortOrder === 'oldest') {
            data.reverse();
        }
        setNotes(data);
    };

    useEffect(() => {
        loadNotes();
    }, [sortOrder]);

    const getReference = (note) => {
        if (!titlesData) return "...";
        const details = getNoteDetails(note);
        const titleObj = titlesData.find(t => String(t.n) == String(details.book));
        const bookName = titleObj
            ? (!getLanguage() || getLanguage() === "Malayalam" ? titleObj.bm : titleObj.be)
            : `Book ${details.book}`;

        const range = formatVerseRange(details.verses);
        return `${bookName} ${details.chapter}:${range}`;
    };

    const getVerseText = (note) => {
        if (!bibleData) return "Loading...";
        try {
            const details = getNoteDetails(note);
            const verses = bibleData.filter(v =>
                Number(v.b) == Number(details.book) &&
                Number(v.c) == Number(details.chapter) &&
                details.verses.includes(Number(v.v))
            ).sort((a, b) => Number(a.v) - Number(b.v));

            if (verses.length === 0) return "";
            if (verses.length === 1) {
                return verses[0].t;
            }
            return verses.map(v => `${v.v}. ${v.t}`).join(' ');
        } catch (e) {
            return "";
        }
    };

    const getNavigationPath = (note) => {
        const details = getNoteDetails(note);
        const startVerse = details.verses[0];
        const endVerse = details.verses[details.verses.length - 1];
        if (startVerse === endVerse) {
            return `/${details.book}/${details.chapter}/${startVerse}`;
        }
        return `/${details.book}/${details.chapter}/${startVerse}-${endVerse}`;
    };

    const toggleExpand = (id) => {
        setExpandedItems(prev => ({
            ...prev,
            [id]: !prev[id]
        }));
    };

    const handleRemove = async (id) => {
        await removeNote(id);
        await loadNotes();
    };

    const handleEdit = (note) => {
        const details = getNoteDetails(note);
        setEditingNote({ ...note, ...details });
        setEditContent(details.noteContent);
    };

    const handleSaveEdit = async () => {
        if (editingNote) {
            const trimmedContent = editContent.trim();
            const details = getNoteDetails(editingNote);
            if (trimmedContent) {
                const start = details.verses[0];
                const end = details.verses[details.verses.length - 1];
                await addNote(
                    details.book,
                    details.chapter,
                    start,
                    end,
                    trimmedContent,
                    details.verses
                );
            } else {
                // Delete note if empty
                await removeNote(editingNote.id);
            }
            setEditingNote(null);
            setEditContent('');
            await loadNotes();
        }
    };

    const baseFontSize = parseInt(localStorage.getItem('fontSize') || 4);
    const noteFontSize = inModal ? 6 : 5;
    const verseFontSize = inModal ? 6 : baseFontSize;

    return (
        <section className={inModal ? "h-100" : "py-4 mb-5"}>
            <div className={inModal ? "" : "container"}>
                {/* Header */}
                {!inModal && (
                    <div className="text-center mb-4">
                        <h2 className="fw-bold">
                            <span style={{ marginRight: '10px' }}>📝</span>
                            Notes
                        </h2>
                        <p className="text-muted">Your personal study notes</p>
                    </div>
                )}

                {/* Search and Filter Bar */}
                <div className="row mb-4 g-3 justify-content-end">
                    <div className="col-md-4">
                        <select
                            className="form-select"
                            value={sortOrder}
                            onChange={(e) => setSortOrder(e.target.value)}
                            style={{ borderRadius: '12px' }}
                        >
                            <option value="newest">Newest First</option>
                            <option value="oldest">Oldest First</option>
                        </select>
                    </div>
                </div>

                {/* Notes List */}
                {isLoading ? (
                    <div className="text-center py-5">
                        <div className="spinner-border text-info" role="status">
                            <span className="visually-hidden">Loading...</span>
                        </div>
                    </div>
                ) : notes.length === 0 ? (
                    <div className="text-center py-5">
                        <div style={{ fontSize: '4rem', opacity: 0.3 }}>📝</div>
                        <p className="text-muted mt-3">No notes yet</p>
                        <Link to="/" className="btn btn-primary mt-2">
                            Start Reading
                        </Link>
                    </div>
                ) : (
                    <div className="row row-cols-1 g-3">
                        {notes.map((note) => (
                            <div key={note.id} className="col">
                                <div
                                    className="card shadow-sm h-100"
                                    style={{
                                        borderRadius: '16px',
                                        overflow: 'hidden',
                                        transition: 'all 0.2s ease',
                                        backgroundColor: 'var(--bs-card-bg)',
                                        border: '1px solid var(--bs-border-color)',
                                        borderLeft: '4px solid #6c757d',
                                    }}
                                >
                                    <div className="card-body p-3">
                                        <div className="d-flex justify-content-between align-items-center mb-2">
                                            <Link
                                                to={getNavigationPath(note)}
                                                className="text-decoration-none"
                                                onClick={onNavigate}
                                            >
                                                <h6 className="fw-bold mb-0 text-secondary">
                                                    {getReference(note)}
                                                </h6>
                                            </Link>
                                            <div className="d-flex gap-2">
                                                <button
                                                    onClick={() => toggleExpand(note.id)}
                                                    className="btn btn-sm btn-outline-secondary border-0"
                                                    title={expandedItems[note.id] ? "Hide Verse" : "View Verse"}
                                                    style={{
                                                        borderRadius: '8px',
                                                        backgroundColor: 'rgba(108, 117, 125, 0.08)',
                                                        fontSize: '0.75rem',
                                                        padding: '4px 8px',
                                                        fontWeight: '500',
                                                    }}
                                                >
                                                    {expandedItems[note.id] ? "Hide" : "View"}
                                                </button>
                                                <button
                                                    onClick={() => handleEdit(note)}
                                                    className="btn btn-sm btn-outline-secondary border-0"
                                                    title="Edit note"
                                                    style={{
                                                        borderRadius: '8px',
                                                        backgroundColor: 'rgba(108, 117, 125, 0.08)',
                                                        padding: '4px 8px',
                                                    }}
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                                                        <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z" />
                                                    </svg>
                                                </button>
                                                <button
                                                    onClick={() => handleRemove(note.id)}
                                                    className="btn btn-sm btn-outline-danger border-0"
                                                    title="Delete note"
                                                    style={{
                                                        borderRadius: '8px',
                                                        backgroundColor: 'rgba(220, 53, 69, 0.08)',
                                                        padding: '4px 8px',
                                                    }}
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                                                        <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z" />
                                                        <path fillRule="evenodd" d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1v1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118zM2.5 3V2h11v1h-11z" />
                                                    </svg>
                                                </button>
                                            </div>
                                        </div>

                                        {/* Note Content */}
                                        <div
                                            className={`p-3 mb-2 fs-${noteFontSize}`}
                                            style={{
                                                backgroundColor: 'rgba(108, 117, 125, 0.05)',
                                                borderRadius: '12px',
                                                border: '1px solid rgba(108, 117, 125, 0.1)'
                                            }}
                                        >
                                            <p className="mb-0" style={{ whiteSpace: 'pre-wrap', fontWeight: '500' }}>
                                                {getNoteDetails(note).noteContent}
                                            </p>
                                        </div>

                                        {/* Verse Text (Collapsible) */}
                                        {expandedItems[note.id] && (
                                            <div
                                                className={`text-muted small mb-2 fs-${verseFontSize} animate-fade-in`}
                                                style={{
                                                    whiteSpace: 'pre-wrap',
                                                    lineHeight: '1.6',
                                                    fontStyle: 'italic',
                                                    backgroundColor: 'rgba(108, 117, 125, 0.05)',
                                                    padding: '12px',
                                                    borderRadius: '12px',
                                                    marginTop: '8px',
                                                    border: '1px solid rgba(108, 117, 125, 0.1)'
                                                }}
                                            >
                                                {getVerseText(note)}
                                            </div>
                                        )}

                                        <div className="mt-2 d-flex justify-content-between align-items-center">
                                            <small className="text-muted" style={{ fontSize: '0.75rem' }}>
                                                Updated on {formatDateTime(getNoteDetails(note).updatedAt)}
                                            </small>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ))}
                    </div>
                )}

                {/* Total count */}
                {notes.length > 0 && (
                    <div className="text-center mt-4">
                        <small className="text-muted">
                            {notes.length} note{notes.length !== 1 ? 's' : ''}
                        </small>
                    </div>
                )}
            </div>

            {/* Edit Note Modal */}
            <Modal show={!!editingNote} onHide={() => setEditingNote(null)} centered>
                <Modal.Header closeButton>
                    <Modal.Title>Edit Note</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    {editingNote && (
                        <>
                            <div className="mb-3">
                                <small className="text-muted">{getReference(editingNote)}</small>
                            </div>
                            <textarea
                                className="form-control"
                                rows={5}
                                value={editContent}
                                onChange={(e) => setEditContent(e.target.value)}
                                autoFocus
                                style={{
                                    borderRadius: '12px',
                                    resize: 'vertical',
                                }}
                            />
                        </>
                    )}
                </Modal.Body>
                <Modal.Footer>
                    <Button variant="secondary" onClick={() => setEditingNote(null)}>
                        Cancel
                    </Button>
                    <Button variant="primary" onClick={handleSaveEdit}>
                        Save Changes
                    </Button>
                </Modal.Footer>
            </Modal>
        </section>
    );
}

export default Notes;
