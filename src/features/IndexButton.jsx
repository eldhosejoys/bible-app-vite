...
function IndexButton() {
  ...
  function numberbox(rp,response) {
    ...
    for (let i = 1; i <= rp["c"]; i++) {
      td.push(
        <div className="numberbox"><Link className="link-dark small text-decoration-none" to={`/${rp["n"]}/${i}`} ><div className="col numberbox">{i}</div></Link> </div>
      );
    }
    let md =[];
  ...
  function biblechapters(response) {
    ...
    response.forEach((rp) => {
     if(c <=39){
      b.push(
        <div className="ps-2 pe-1 fs-5" onClick={() => numberbox(rp,response)} style={{backgroundColor:''}} onMouseOver={(event) => { event.target.style.backgroundColor = '#0275d8' }} onMouseOut={(event) => { event.target.style.backgroundColor = '' }}>{rp[h_lang]}</div>
      );
     }else{
      b2.push(
        <div className="ps-2 pe-1 fs-5" onClick={() => numberbox(rp,response)} style={{backgroundColor:''}} onMouseOver={(event) => { event.target.style.backgroundColor = '#0275d8' }} onMouseOut={(event) => { event.target.style.backgroundColor = '' }}>{rp[h_lang]}</div>
